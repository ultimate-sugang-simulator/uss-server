...
--              @cart_per_member, @registration_per_member
-- 선행 모듈: member.sql, course.sql
--
-- 두 테이블 모두 UNIQUE KEY uk_member_course (member_id, course_id)를 가진다.
-- 회원마다 서로 다른 강의를 배정해야 하므로 (m * 37 + k) % @course_count로 흩는다.
--
-- **쓰기 엔드포인트를 측정할 때 주의한다.**
-- POST /api/v1/carts/{courseId}나 POST /api/v1/registration/{courseId}를 재는 경우,
-- 부하가 담으려는 강의를 여기서 미리 채워두면 전부 중복으로 실패한다.
-- 그 대상이면 @cart_per_member 또는 @registration_per_member를 0으로 두고,
-- 조회 대상일 때만 채운다.
...
-- current_enrollment를 실제 신청 건수와 맞춘다.
-- 이 값이 어긋나 있으면 정원 판정이 실제 데이터와 다르게 동작해, 측정에서 재는 분기가 달라진다.
UPDATE courses c
SET c.current_enrollment = (
    SELECT count(*) FROM registrations r WHERE r.course_id = c.id
)
...
-- 장바구니 상한(10)을 넘긴 회원이 있으면 안 된다. 넘겼다면 그 회원은 담기 API가 항상 실패한다.
...
-- current_enrollment가 정원을 넘긴 강의가 있으면 안 된다. 넘겼다면 신청 API가 항상 마감으로 실패한다.
