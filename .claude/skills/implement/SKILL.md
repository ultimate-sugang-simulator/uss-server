...
5. 구현 결과로 서비스 정책이 바뀌거나 새 정책이 생기면 `.claude/spec/service-policy/`의 해당 도메인 파일을 같은 작업에서 수정하라.
   - 성공과 실패 조건, 제한 수치, 산정 기준, 검증 순서, 유효 기간, 권한 범위가 달라졌다면 정책 변경이다.
   - 코드만 바꾸고 문서를 두고 가지 마라. 문서가 코드와 어긋나는 순간 SSOT가 아니다.

> 다음 Phase 조건: 계획서의 모든 항목이 구현되었을 때.
> 계획서 "영향 범위 - 수정 파일"에 정책 파일이 있으면 그 파일이 실제로 갱신되었을 때
